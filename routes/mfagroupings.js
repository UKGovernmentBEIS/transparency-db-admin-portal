...
router.get("/", async (req, res) => {
  ...
  if (
    ...
    if (req.query.hasOwnProperty("mfaGroupingName")){
      ssn.MFA_Grouping_Search_Name_Global = req.query.mfaGroupingName;
    } else {
      ssn.MFA_Grouping_Search_Name_Global = "";
    }
    ...
    if (req.query.hasOwnProperty("status")) {
      mfaGroupingStatus = req.query.status;
    } else {
      mfaGroupingStatus = "";
    }
    ...
    const data_request = {
      searchName: ssn.MFA_Grouping_Search_Name_Global,
    ...
    try {
      ...
      ssn.allMfaGroupings = searchResults.allMfaGroupings;
      ssn.activeMfaGroupings = searchResults.activeMfaGroupings;
      ssn.deletedMfaGroupings = searchResults.deletedMfaGroupings;
      ...
      if (err.toString().includes("404")) {
        noresult = true;
        nodata = "No MFA grouping available";
        res.render("mfa/mfagroupings", {
          noresult,
          nodata,
        });
