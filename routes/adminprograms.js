...
router.get("/", async (req, res) => {
  ...
  if (
    ...
    if (req.query.hasOwnProperty("query")){
      ssn.Admin_Program_Search_Name_Global = req.query.query;
    } else {
      ssn.Admin_Program_Search_Name_Global = "";
    }
    ...
    if (req.query.hasOwnProperty("status")) {
      adminProgramStatus = req.query.status;
    } else {
      adminProgramStatus = "";
    }
    ...
    const data_request = {
      searchName: ssn.Admin_Program_Search_Name_Global,
    ...
    try {
      const apidata = await axios.post(
        beis_url_searchscheme + "/adminprogram/search",
        data_request,
      ...
      ssn.allAdminPrograms = searchResults.allAdminPrograms;
      ssn.activeAdminPrograms = searchResults.activeAdminPrograms;
      ssn.deletedAdminPrograms = searchResults.deletedAdminPrograms;
      ...
      nodata = "";
      noresult = false;
      res.render("admin-program/adminprograms", {
        pageCount,
        previous_page,
        next_page,
        start_record,
      ...
      if (err.toString().includes("404")) {
        noresult = true;
        nodata = "No admin programs available";
        res.render("admin-program/adminprograms", {
          noresult,
          nodata,
        });
