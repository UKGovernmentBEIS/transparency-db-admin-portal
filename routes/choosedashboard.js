...
router.post("/", async (req, res) => {
  ...
  if (dashboard_roles == "BEIS Administrator") {
    ...
    const userPrincipleRequest1 = {
      userName: "TEST",
      password: "password123",
      role: "BEIS Administrator",
      grantingAuthorityGroupId: "123",
      grantingAuthorityGroupName: "test",
    };
    ...
    try {
      const apidata = await axios.get(
        "https://access-management-service.azurewebsites.net/accessmanagement/beisadmin",
        config
      );
    ...
    const userPrincipleRequest =
      '{"userName":"SYSTEM","password":"password123","role":"Granting Authority Administrator","grantingAuthorityGroupId":"123","grantingAuthorityGroupName":"DEFRA"}';
    var config = {
      headers: { userPrinciple: userPrincipleRequest },
    };
    ...
    try {
      const apidata = await axios.get(
        "https://access-management-service.azurewebsites.net/accessmanagement/gaadmin",
        config
      );
      console.log(`Status: ${apidata.status}`);
      API_response_code = `${apidata.status}`;
  ...
  } else if (dashboard_username == "Granting Authority Encoder") {
